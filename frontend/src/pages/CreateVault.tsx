...
export default function CreateVault() {
  ...
  const handleTokenToggle = (index: number) => {
    setTokens(prev => prev.map((token, i) =>
      i === index
        ? { ...token, isSelected: !token.isSelected, percentage: token.isSelected ? 0 : 25 }
        : token
    ));
  };
  ...
  const handlePercentageChange = (index: number, percentage: number) => {
    ...
    setTokens(prev => prev.map((token, i) =>
      i === index ? { ...token, percentage } : token
    ));
  };
  ...
  const autoBalancePercentages = () => {
    ...
    setTokens(prev => prev.map(token => {
      ...
      const index = selectedTokens.findIndex(st => st.address === token.address);
  ...
  const handleCreateVault = async () => {
    ...
    try {
      ...
      const tokensWithPercentage = tokens
        .filter(token => token.isSelected)
        .map(token => new TokenWithPercentage(token.address, BigInt(token.percentage)));

      const result = await createSplitterVault(connectedAccount, tokensWithPercentage);
        ...
        <div className="space-y-4">
          ...
          <div className="brut-card bg-gray-50 p-4">
            ...
            <div className="mb-3">
              ...
              <div className="mt-2 space-y-2">
                {tokens.filter(token => token.isSelected).map(token => (
          ...
          {!connectedAccount && (
            <div className="brut-card bg-red-100 p-4">
              <p className="text-red-700 font-bold">Please connect your wallet to deploy the vault</p>
            </div>
          )}
        </div>
      )}
      ...
      <div className="mt-6 flex gap-3">
        ...
        {step < 2 ? (
          <button 
            ...
            disabled={(step === 1 && (!hasSelectedTokens || !isValidPercentages)) || loading}
          >
            Next
          </button>
        ) : (
          <button
            ...
            disabled={!connectedAccount || !hasSelectedTokens || !isValidPercentages || loading}
          >
